...
public class Sender
{
    ...
    {
        // First we need to initialise the Node. Start listening process on port 8000 of this machine
        Node.getInstance().init(new TCPIPNodeAddress(8000));

        // We now need to create a channel to the receiver process. It is listening on port 8081 on this machine, and is
        // using channel 100
        NetChannelOutput out = NetChannel.one2net(new TCPIPNodeAddress(8081), 100);
